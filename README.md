...
2. 解压得到 `FocusGuard.app`，**拖入「应用程序」文件夹**
3. **首次打开时，右键点 App → 按住 Option → 选「打开」**（这是 macOS 对未签名 App 的安全拦截，做一次就好）

> 首次运行会请求管理员权限安装后台助手，安装后屏蔽操作静默执行。

### 如果提示「已损坏，无法打开」

这是 macOS Gatekeeper 拦截未签名 App，不是真的损坏。右键打开仍不行的话，在终端运行：
