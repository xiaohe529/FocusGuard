...
1. 从 [GitHub](https://github.com/xiaohe529/FocusGuard/releases) 或 [Gitee](https://gitee.com/xiaohe529/focus-guard/releases) 下载 `FocusGuard-v*.pkg`
2. 双击 `.pkg`，按安装器向导一路下一步即可

> 首次运行会请求管理员权限安装后台助手，安装后屏蔽操作静默执行。

### 如果提示「无法验证开发者」

FocusGuard 是开源免费软件，没有购买 Apple 付费开发者签名。遇到拦截时：

- **pkg 安装器**：右键点 `.pkg` → 选「打开」，系统会多一个「打开」按钮
- **还不行**：去「系统设置 → 隐私与安全性」底部，会看到 "FocusGuard.pkg 已被阻止" → 点「仍要打开」
...
├── make-release.sh          # .app → 发布用 pkg 安装包
└── LICENSE
```
